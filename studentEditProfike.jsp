<%-- 
    Document   : studentEditProfike
    Created on : 12 Apr, 2023, 11:38:58 AM
    Author     : Deeksha
--%>
...
<body>
    <form action="studentEditProfike.jsp" method="post">
        <h1>Edit Profile</h1>
    ...
    <% 
        if (request.getParameter("ok") != null) { // Check if the form is submitted
            ...
            String query = "SELECT * FROM StudentRegistration WHERE smartid=?";
            PreparedStatement stmt = conn.prepareStatement(query);
            stmt.setString(1, id);
            ResultSet rs = stmt.executeQuery();
            if (rs.next()) {
                String name = rs.getString("sname");
                String email = rs.getString("smail");
                String pass = rs.getString("spassword");
                String dept = rs.getString("sdepartment");
                // You can fetch other fields from the table similarly

                // Display the fetched data in the edit profile form
    %>
                <form action="studentEditProfike.jsp" method="post"> <!-- Update with the URL of the update profile logic -->
    ...
    <%
        ...
        if (request.getParameter("id") != null && request.getParameter("name") != null && request.getParameter("email") != null && request.getParameter("pass") != null && request.getParameter("dept") != null) {
            ...
            String query = "UPDATE StudentRegistration SET sname=?, smail=?, spassword=?, sdepartment=? WHERE smartid=?";
            PreparedStatement stmt = conn.prepareStatement(query);
